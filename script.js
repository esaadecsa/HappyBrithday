...
function initTimeCapsule() {
  ...
  const showLocked = () => {
    ...
    if (heading) heading.innerHTML = "Ini sudah sampai,<br><em>tapi belum boleh dibuka.</em>";
    if (sub) sub.textContent = "Sebuah hadiah kecil sedang menunggu hari spesialmu. Balik lagi ke sini ya.";
  ...
  if (isTimeCapsuleUnlocked()) { showUnlocked(); return; }
  showLocked();
...
function confetti() {
  ...
  const colors = ["#c9a15a", "#eecd8e", "#e0899f", "#f7ece7", "#9c3f68", "#f5e6c0"];
  for (let i = 0; i < 150; i++) {
...
function nextPinRequestDate() {
  ...
  const target = new Date(`${year}-09-01T00:00:00+07:00`);
  return now >= target ? new Date(`${year + 1}-09-01T00:00:00+07:00`) : target;
}
