...
const CONFIG = {
  ...
  // MEWAKILI lagu yang sama dengan yang sudah main otomatis di layar PIN
  // (elemen <audio id="pin-music">) — jadi src di sini cuma buat catatan,
  // pemutarannya tetap pakai elemen pin-music yang sama supaya lagunya
  // TIDAK pernah restart/putus waktu masuk ke slide ini. Ganti title/artist
  // di bawah sesuai lagu pin-song.mp3 kamu.
  // Item 1-4 baru beneran diputar lewat elemen <audio id="background-music">.
  playlist: [
    { id: "pinsong", title: "Spirit In The Sky", artist: "MAAPS", src: "./music/pin-song.mp3", hue: 210 },
...
// Dideklarasikan di sini (bukan di bawah dekat startPinMusic/stopPinMusic)
// karena item pertama di slide playlist sudah butuh elemen ini sejak awal
// load (initPlaylistDefaultSelection di bawah). Kalau tetap di bawah,
// dipakai sebelum dideklarasikan -> ReferenceError -> seluruh script
// berhenti jalan, termasuk handler input PIN.
...
function showScreen(name) {
  ...
  currentScreen = name;
  // Jangan langsung matikan musik PIN begitu masuk ke slide playlist —
  // item pertama di playlist MEWAKILI lagu ini juga (masih pakai elemen
  // pin-music yang sama), jadi biar tidak restart/putus, cuma dihentikan
  // kalau pengunjung sendiri memilih lagu lain di daftar (lihat selectSong()).
  if (previousScreen === "pin" && name !== "pin" && name !== "playlist") stopPinMusic();
...
// Item index 0 = lagu yang sama dengan yang sudah main dari layar PIN
// (elemen pin-music), jadi statusnya dicek dari pinMusic, bukan music.
function isSongCurrentlyPlaying(i) {
  if (i === 0) return selectedSongIndex === 0 && !!pinMusic && !pinMusic.paused;
...
function selectSong(i) {
  ...
  if (i === 0) {
    // Balik/tap lagi ke item lagu PIN: kalau sedang jadi pilihan aktif,
    // tap = jeda/lanjut. Kalau lagi pilih lagu lain, pindah balik ke sini
    // berarti menghentikan lagu playlist & melanjutkan lagu PIN (bukan
    // memutar ulang dari awal — pinMusic cuma di-pause tadi, bukan di-stop).
    if (selectedSongIndex === 0) {
      if (!pinMusic.paused) { pinMusic.pause(); } else { pinMusic.play().catch(() => {}); }
      updatePlaylistPlayMarks();
      return;
    }
  ...
  // Pilih salah satu lagu playlist (index 1+): pastikan lagu PIN berhenti
  // dulu (di-pause, bukan di-restart) supaya tidak dobel suara.
  if (!pinMusic.paused) pinMusic.pause();
...
// Jaring pengaman: elemen pinMusic dan music TIDAK BOLEH main bersamaan
// (itu yang bikin kedengaran "dobel"/nabrak). Begitu salah satu mulai
// main, langsung jeda yang satunya lagi, apa pun jalur kode yang memicunya.
music.addEventListener("play", () => { if (pinMusic && !pinMusic.paused) pinMusic.pause(); });
...
// Item pertama di slide playlist memakai elemen ini juga (lihat selectSong
// di atas), jadi penanda "sedang diputar"/ikon jeda-lanjut di situ perlu
// ikut ter-update setiap kali pinMusic main/berhenti, bukan cuma "music".
pinMusic?.addEventListener("play", updatePlaylistPlayMarks);
pinMusic?.addEventListener("pause", updatePlaylistPlayMarks);
// Kebalikan dari jaring pengaman di atas: kalau pinMusic yang mulai main,
// pastikan music (elemen playlist utama) ikut dijeda juga.
pinMusic?.addEventListener("play", () => { if (!music.paused) music.pause(); });
