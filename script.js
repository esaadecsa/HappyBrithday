...
const PLANET_PROFILES = [
  { name: "sun", bg: "radial-gradient(circle at 34% 30%, #fff6d2, #ffcf5e 45%, #ff9636 100%)", glow: "rgba(255,190,90,.55)", ring: false, sizeMul: 1.15 },
  { name: "mercury", bg: "radial-gradient(circle at 36% 30%, #d3c9c0, #8d8078 60%, #55483f 100%)", glow: "rgba(180,170,160,.3)", ring: false, sizeMul: .5 },
  { name: "venus", bg: "radial-gradient(circle at 36% 30%, #ffe9b8, #e8b970 55%, #a97a3d 100%)", glow: "rgba(255,210,140,.35)", ring: false, sizeMul: .7 },
  { name: "earth", bg: "radial-gradient(circle at 36% 30%, #cdeeff, #4f9fd8 45%, #2c6b4f 100%)", glow: "rgba(107,180,220,.4)", ring: false, sizeMul: .78 },
  { name: "mars", bg: "radial-gradient(circle at 36% 30%, #ffb98f, #c9542b 55%, #7a2c14 100%)", glow: "rgba(220,110,70,.35)", ring: false, sizeMul: .58 },
  { name: "jupiter", bg: "radial-gradient(circle at 34% 28%, #f0d9b0, #c98f52 40%, #8a5a2e 75%, #5c3a1e 100%)", glow: "rgba(210,160,100,.35)", ring: false, sizeMul: 1.3 },
  { name: "saturn", bg: "radial-gradient(circle at 36% 30%, #f7e7bd, #d8bf7e 55%, #a3813f 100%)", glow: "rgba(230,200,140,.35)", ring: true, sizeMul: 1.05 },
  { name: "uranus", bg: "radial-gradient(circle at 36% 30%, #d9fbfa, #8fd9d6 55%, #4f9c9a 100%)", glow: "rgba(140,220,215,.35)", ring: true, sizeMul: .8 },
  { name: "neptune", bg: "radial-gradient(circle at 36% 30%, #b9c8ff, #4a5fc9 55%, #29357e 100%)", glow: "rgba(110,130,220,.35)", ring: false, sizeMul: .78 }
];
const planetSlots = [
  { el: $("#dpA"), body: $("#dpA")?.querySelector(".planet-body"), base: 190, current: null },
  { el: $("#dpB"), body: $("#dpB")?.querySelector(".planet-body"), base: 130, current: null },
  { el: $("#dpC"), body: $("#dpC")?.querySelector(".planet-body"), base: 56, current: null }
];
...
function applyPlanetProfile(slot, profile) {
  const el = slot.el, body = slot.body;
  if (!el || !body) return;
  const size = Math.round(slot.base * profile.sizeMul);
  el.style.width = size + "px";
  el.style.height = size + "px";
  body.style.background = profile.bg;
  body.style.boxShadow = `0 0 ${Math.round(size * .36)}px ${profile.glow}`;
  const ring = body.querySelector(".planet-ring");
  if (ring) ring.classList.toggle("show", !!profile.ring);
  slot.current = profile.name;
}
planetSlots.forEach((slot) => { if (slot.el) applyPlanetProfile(slot, randomPlanetProfile(null)); });
/* ---------- swapRandomPlanet — sekarang planet tidak menghilang
   dengan fade diam di tempat: ia bergerak menjauh (kelas "swapping")
   sampai betul-betul hilang dari pandangan, baru wujud barunya
   dipasang lalu "masuk" dengan bergerak dari sisi yang berlawanan
   (kelas "incoming") menuju posisi normalnya. ---------- */
function swapRandomPlanet() {
  const candidates = planetSlots.filter((s) => s.el && s.body);
  if (!candidates.length) return;
  const slot = candidates[Math.floor(Math.random() * candidates.length)];
  slot.el.classList.add("swapping"); // planet lama kabur bergerak menjauh sambil memudar
  setTimeout(() => {
    applyPlanetProfile(slot, randomPlanetProfile(slot.current));
    slot.el.classList.remove("swapping");
    slot.el.classList.add("incoming"); // langsung lompat ke posisi "belum sampai" tanpa transisi
    void slot.body.offsetWidth; // paksa reflow biar transisi masuk berikutnya benar-benar kepakai
    slot.el.classList.remove("incoming"); // lalu bergerak masuk ke posisi normal
  }, 680);
}
setInterval(swapRandomPlanet, 9000);
