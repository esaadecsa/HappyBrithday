...
// true kalau pengunjung memilih tetap pakai lagu yang sudah main dari layar
// PIN (bukan lagu dari playlist di bawah). Dipakai buat gerbang tombol
// "Lanjutkan" di slide playlist, sama seperti selectedSongIndex.
let usingPinSong = false;
...
function showScreen(name) {
  ...
  // biarkan tetap main sampai pengunjung benar-benar memilih (tetap lagu
  // ini / ganti lagu) lewat choosePinSong()/openSongPicker() di bawah.
...
/* ---------- pilihan di slide playlist: tetap lagu dari PIN, atau ganti
   lewat playlist di bawah. Lagu yang sedang main tidak pernah diputus
   paksa oleh perpindahan slide — cuma diputus kalau pengunjung sendiri
   memilih ganti/pilih lagu lain. ---------- */
function showPlaylistNext(label) {
  const nextBtn = $("#playlistNextBtn");
  if (nextBtn) { nextBtn.hidden = false; nextBtn.disabled = false; }
  const now = $("#playlistNow"), nowText = $("#playlistNowText");
  if (now && nowText) { now.style.display = "flex"; nowText.textContent = `Sedang diputar: ${label}`; }
}
function choosePinSong() {
  usingPinSong = true;
  selectedSongIndex = null;
  document.querySelectorAll(".playlist-item").forEach((el) => el.classList.remove("picked"));
  if (!music.paused) music.pause();
  $("#playlistChoice")?.setAttribute("hidden", "");
  $("#playlistPicker")?.setAttribute("hidden", "");
  showPlaylistNext("lagu dari tadi");
}
function openSongPicker() {
  usingPinSong = false;
  stopPinMusic();
  $("#playlistChoice")?.setAttribute("hidden", "");
  $("#playlistPicker")?.removeAttribute("hidden");
  const nextBtn = $("#playlistNextBtn");
  if (nextBtn) { nextBtn.hidden = false; nextBtn.disabled = selectedSongIndex === null; }
  if (selectedSongIndex === null) {
    const now = $("#playlistNow");
    if (now) now.style.display = "none";
  }
}
function backToPinSong() {
  if (!music.paused) music.pause();
  selectedSongIndex = null;
  document.querySelectorAll(".playlist-item").forEach((el) => el.classList.remove("picked"));
  startPinMusic();
...
$("#keepPinSongBtn")?.addEventListener("click", () => { playSfx("tap"); choosePinSong(); });
$("#changeSongBtn")?.addEventListener("click", () => { playSfx("tap"); openSongPicker(); });
$("#backToPinSongBtn")?.addEventListener("click", () => { playSfx("tap"); backToPinSong(); });
...
function primaryAction() {
  ...
  if (currentScreen === "playlist") return (selectedSongIndex !== null || usingPinSong) && goMinigame();
