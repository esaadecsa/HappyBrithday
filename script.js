...
const screens = {
  intro: $("#intro"), letter: $("#letter"), wish: $("#wish"), memories: $("#memories"), gift: $("#gift"),
  nope: $("#nope"), candle: $("#candle"), flower: $("#flower-screen"), final: $("#final")
};
const SCREEN_ORDER = ["intro", "letter", "wish", "memories", "gift", "candle", "flower", "final"];
...
function primaryAction() {
  ...
  if (currentScreen === "wish") return goMemories();
  if (currentScreen === "memories") return goGift();
  if (currentScreen === "flower") return goFinal();
}
document.addEventListener("keydown", (e) => {
  if (["ArrowRight", "Enter", " "].includes(e.key)) {
    if (["intro", "letter", "wish", "memories", "flower"].includes(currentScreen)) { e.preventDefault(); primaryAction(); }
  }
...
document.addEventListener("touchend", (e) => {
  ...
  if (Math.abs(dx) > 60 && Math.abs(dx) > Math.abs(dy) * 1.5 && dx < 0) {
    if (["intro", "letter", "wish", "memories", "flower"].includes(currentScreen)) primaryAction();
  }
}, { passive: true });
...
$("#giftBtn").onclick = () => { playSfx("tap"); goMemories(); };
$("#memoriesNextBtn").onclick = () => { playSfx("tap"); goGift(); };
