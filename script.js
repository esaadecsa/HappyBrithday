...
const SCREEN_ORDER = ["playlist", "minigame", "intro", "letter", "wish", "mood", "spirit", "memories", "polaroid", "gift", "candle", "flower", "farewell", "final", "reaction", "reply"];
...
$("#sendReplyBtn").onclick = async () => {
  ...
  status.textContent = ok ? "balasanmu udah nyampe ke aku ♡" : "tersimpan, tapi belum sempat terkirim — coba lagi nanti";
  playSfx("chime");
  vibrate(16);
};

/* ---------- story card: gambar format story (potret) buat di-share native ---------- */
async function buildStoryCardBlob() {
  try {
    if (document.fonts && document.fonts.load) {
      await Promise.all([
  ...
  } catch (e) { /* fallback font di bawah */ }
  ...
  const bg = ctx.createLinearGradient(0, 0, W, H);
  bg.addColorStop(0, "#180a12"); bg.addColorStop(.55, "#20111a"); bg.addColorStop(1, "#0e0710");
  ctx.fillStyle = bg; ctx.fillRect(0, 0, W, H);
  ...
  for (let i = 0; i < 160; i++) {
    ...
    ctx.fillStyle = "#f4ead9";
    const s = Math.random() * 2 + .5;
    ctx.beginPath(); ctx.arc(Math.random() * W, Math.random() * H * .75, s, 0, Math.PI * 2); ctx.fill();
  }
  ctx.globalAlpha = 1;

  const glow = ctx.createRadialGradient(W * .5, H * .32, 10, W * .5, H * .32, 620);
  glow.addColorStop(0, "rgba(227,166,189,.30)"); glow.addColorStop(1, "rgba(227,166,189,0)");
  ctx.fillStyle = glow; ctx.fillRect(0, 0, W, H);

  ctx.textAlign = "center";
  ctx.fillStyle = "#e3a6bd";
  ctx.font = `600 26px "${bodyFont}"`;
  ctx.fillText("BARU DAPET KEJUTAN KECIL", W / 2, 420);

  const emoji = REACTION_EFFECTS[lastPickedReaction] ? (document.querySelector(`.reaction-btn[data-reaction="${lastPickedReaction}"]`)?.dataset.emoji || "♡") : "♡";
  ctx.font = `170px "${bodyFont}"`;
  ctx.fillText(emoji, W / 2, 660);

  ctx.fillStyle = "#f4ead9";
  ctx.font = `italic 600 78px "${displayFont}"`;
  ctx.fillText("Ada surat kecil", W / 2, 840);
  ctx.fillStyle = "#f0dcab";
  ctx.font = `italic 600 84px "${displayFont}"`;
  ctx.fillText("untuk aku ♡", W / 2, 940);

  ctx.strokeStyle = "rgba(201,164,99,.3)";
  ctx.lineWidth = 1;
  ctx.beginPath(); ctx.moveTo(W * .3, 1040); ctx.lineTo(W * .7, 1040); ctx.stroke();

  ctx.fillStyle = "#cabfae";
  ctx.font = `30px "${bodyFont}"`;
  wrapCanvasText(ctx, "Semoga kamu juga dapet hari yang baik hari ini.", W / 2, 1140, 820, 44);

  ctx.fillStyle = "#8a8070";
  ctx.font = `26px "${bodyFont}"`;
  ctx.fillText("esaadecsa.github.io/HappyBrithday", W / 2, H - 140);
...
$("#storyShareBtn").onclick = async () => {
  ...
  if (navigator.canShare && navigator.canShare({ files: [file] })) {
    try {
      await navigator.share({ files: [file], title: "For You ♡", text: "Story kecil buat dibagikan ✦" });
      status.textContent = "story-nya siap dibagikan ✦";
      return;
  ...
  const link = document.createElement("a");
  link.download = "story-birthday.png";
