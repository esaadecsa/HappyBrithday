...
async function drawStoryCard(ctx, W, H) {
  ...
  // base gradient — sesuai palet situs (deep plum → berry)
  const bg = ctx.createLinearGradient(0, 0, W, H);
  bg.addColorStop(0, "#1a0f16"); bg.addColorStop(.5, "#20111a"); bg.addColorStop(1, "#0e0710");
  ctx.fillStyle = bg; ctx.fillRect(0, 0, W, H);

  // soft mesh blobs buat kedalaman warna
  const blob1 = ctx.createRadialGradient(W * .18, H * .14, 10, W * .18, H * .14, 520);
  blob1.addColorStop(0, "rgba(227,166,189,.32)"); blob1.addColorStop(1, "rgba(227,166,189,0)");
  ctx.fillStyle = blob1; ctx.fillRect(0, 0, W, H);
  const blob2 = ctx.createRadialGradient(W * .85, H * .88, 10, W * .85, H * .88, 560);
  blob2.addColorStop(0, "rgba(201,164,99,.22)"); blob2.addColorStop(1, "rgba(201,164,99,0)");
  ...
  badgeGlow.addColorStop(0, "rgba(227,166,189,.35)"); badgeGlow.addColorStop(1, "rgba(227,166,189,0)");
  ...
  ctx.lineWidth = 2; ctx.strokeStyle = "rgba(255,217,230,.55)"; ctx.stroke();
  ctx.font = `128px "${bodyFont}"`;
  ctx.fillText(emoji, W / 2, badgeCy + 46);

  // judul
  ctx.fillStyle = "#faeef2";
  ...
  ctx.strokeStyle = "rgba(201,164,99,.35)";
  ctx.lineWidth = 1;
