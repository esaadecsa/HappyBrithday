...
// Satu link buat semua orang sekarang (nggak perlu ?to= beda-beda lagi —
// yang bisa lanjut cuma yang tahu PIN, lihat layar PIN). ?date= tetap
// didukung kalau suatu saat kamu mau override tanggal tanpa edit kode.
...
function initScratchCard() {
  ...
  function scratchAt(x, y) {
    ...
    // Stroke a thick round-cap line from the last point to this one
    // (not just a dab at each sampled point) so fast, ordinary-speed
    // scratching still erases a continuous swath instead of a dotted
    // trail with gaps left between pointer/touch samples.
  ...
  // Root cause of both "scratch leaves patches" and "screen can be dragged":
  // the card sits inside a screen with overflow-y:auto, and some in-app
  // browsers (e.g. WhatsApp/Instagram's built-in webview, since this link
  // is shared via WhatsApp) don't fully honor touch-action:none. If the
  // page scrolls even a few px mid-stroke, the canvas's bounding rect moves
  // under the finger, so scratchAt() keeps drawing at coordinates that no
  // longer match where the finger actually is — leaving unscratched gaps
  // while it visibly feels like the whole screen is being dragged.
  // Fix: hard-freeze the ancestor scroller for the duration of the stroke,
  // preventDefault as a non-passive fallback, and use pointer capture so
  // fast strokes don't lose tracking once they leave the small canvas box.
  ...
  // Touch listeners kept as a fallback for webviews with flaky Pointer
  // Events support; not passive so preventDefault can actually stop the
  // page from scrolling mid-stroke.
...
document.addEventListener("touchstart", (e) => {
  // Ignore drag gestures that start on interactive/draggable widgets —
  // otherwise scratching the secret card, dragging the polaroid
  // carousel, or writing in a textarea could be misread as a
  // left/right swipe and yank the whole screen to the next page
  // mid-gesture (this is what caused the scratch card to visibly
  // jump/shift screens while being scratched).
  if (e.target.closest("#scratchCanvas, .polaroid-stack, textarea, input, .playlist-list, .memory-card")) { touchStartX = null; return; }
...
function confetti() {
  ...
  const colors = ["#f2b134", "#e8823c", "#c1442e", "#f6ecd9", "#8a5a2b", "#d99a3d"];
  for (let i = 0; i < 150; i++) {
...
function burstMiniConfetti(anchor, colors) {
  ...
  const layer = $("#confetti");
  const palette = colors || ["#f2b134", "#e8823c", "#c1442e", "#f6ecd9", "#8a5a2b", "#d99a3d"];
  for (let i = 0; i < 26; i++) {
    const e = document.createElement("i"); e.className = "confetti";
...
function updatePinRequestCountdown() {
  ...
  if (!el) return;
  const now = new Date();
  const target = nextPinRequestDate();
  const diffMs = target - now;
  if (diffMs <= 0) {
    el.innerHTML = "✦ sekarang udah bisa minta PIN-nya ✦";
    el.classList.add("today");
    return;
  }
  el.classList.remove("today");
  ...
  const hours = Math.floor((diffMs % 86400000) / 3600000);
  const minutes = Math.floor((diffMs % 3600000) / 60000);
  if (days > 0) el.innerHTML = `<b>${days}</b> hari <b>${hours}</b> jam lagi`;
  else if (hours > 0) el.innerHTML = `<b>${hours}</b> jam <b>${minutes}</b> menit lagi`;
  else el.innerHTML = `<b>${Math.max(minutes, 1)}</b> menit lagi`;
}
updatePinRequestCountdown();
setInterval(updatePinRequestCountdown, 60000);
