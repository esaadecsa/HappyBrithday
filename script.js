...
/* =========================================================
   AMBIENT LAYERS — drifting space debris, shooting stars, starfield
   ========================================================= */
/* Dulu hati beterbangan; sekarang partikel angkasa (planet mini,
   asteroid tumbling, satelit berkedip) supaya senada dengan tema
   galaxy/bintang jatuh/meteor yang sudah ada di halaman lain. */
function spawnSpaceDrift() {
  const layer = $("#hearts");
  if (!layer) return;
  const roll = Math.random();
  const type = roll < 0.45 ? "asteroid" : roll < 0.75 ? "planet" : "satellite";
  const e = document.createElement("span");
  e.className = "drift-item drift-" + type;

  let w, h, spin;
  if (type === "planet") {
    w = h = 14 + Math.random() * 14;
    spin = Math.random() * 30 - 15; // planet berputar halus
    if (Math.random() < 0.55) e.classList.add("has-ring");
  } else if (type === "asteroid") {
    w = 8 + Math.random() * 10;
    h = w * (0.75 + Math.random() * 0.3);
    spin = (Math.random() < 0.5 ? -1 : 1) * (540 + Math.random() * 540); // tumbling berkali-kali putaran
  } else {
    w = 9 + Math.random() * 5;
    h = w * 0.6;
    spin = Math.random() * 70 - 35; // satelit goyang ringan
  }
  e.style.width = w + "px";
  e.style.height = h + "px";
  e.style.left = Math.random() * 100 + "vw";
  e.style.setProperty("--drift", (Math.random() * 140 - 70) + "px");
  e.style.setProperty("--spin", spin + "deg");
  const dur = 10 + Math.random() * 9;
  e.style.animationDuration = dur + "s";
  layer.appendChild(e);
  setTimeout(() => e.remove(), dur * 1000 + 200);
}
setInterval(spawnSpaceDrift, 1700);
for (let i = 0; i < 4; i++) setTimeout(spawnSpaceDrift, i * 400);
