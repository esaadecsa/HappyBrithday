...
async function drawStoryCard(ctx, W, H) {
  ...
  const cardX = 90, cardY = 300, cardW = W - 180, cardH = 1260, cardR = 40;
  ctx.save();
  ...
  // judul — beda copy buat sang target ulang tahun vs pengunjung umum
  const targetView = isTargetViewer();
  ctx.fillStyle = theme.title;
  ctx.font = `italic 600 76px "${displayFont}"`;
  ctx.fillText(targetView ? `Buat ${CONFIG.name}` : "Ada hadiah kecil", W / 2, cardY + 500);
  ctx.fillStyle = theme.titleAccent;
  ctx.font = `italic 600 68px "${displayFont}"`;
  ctx.fillText(targetView ? "ada momen kecil hari ini ♡" : "khusus untuk satu orang ♡", W / 2, cardY + 580);
  ...
  // footer / signature — beda juga antara target vs pengunjung umum
  ctx.fillStyle = customBgImage ? "rgba(255,255,255,.75)" : "#cabfae";
  ctx.font = `600 26px "${bodyFont}"`;
  ctx.fillText(targetView ? `— ${CONFIG.name}` : "satu untuk kamu juga? ✦", W / 2, cardY + cardH - 80);

  // hati-hati kecil melayang buat sentuhan playful
  ctx.font = `40px "${bodyFont}"`;
  ctx.fillStyle = "rgba(255,255,255,.4)";
  ctx.fillText("♡", cardX + 24, cardY - 30);
  ctx.fillText("✦", cardX + cardW - 24, cardY + cardH + 40);

  // ---------- barcode (QR) + link, di bawah kartu ----------
  // QR selalu mengarah ke link yang lagi dipakai sekarang (shareUrl()),
  // jadi kalau ini poster milik target, QR-nya otomatis personal;
  // kalau pengunjung umum, QR mengarah ke link generik yang mereka pakai.
  const qrSize = 130;
  const qrTop = cardY + cardH + 70;
  const qr = await getQrCanvas(shareUrl(), 300);
  if (qr) {
    const qx = W / 2 - qrSize / 2;
    roundRectPath(ctx, qx - 12, qrTop - 12, qrSize + 24, qrSize + 24, 16);
    ctx.fillStyle = "#ffffff"; ctx.fill();
    ctx.strokeStyle = "rgba(255,255,255,.4)"; ctx.lineWidth = 1.5; ctx.stroke();
    ctx.drawImage(qr, qx, qrTop, qrSize, qrSize);
  }
  ctx.fillStyle = theme.eyebrow;
  ctx.font = `600 20px "${bodyFont}"`;
  ctx.fillText("✦ SCAN BUAT BUKA ✦", W / 2, qrTop + qrSize + 36);
  ...
  ctx.fillText(pillText, W / 2, pillY + 36);
}
