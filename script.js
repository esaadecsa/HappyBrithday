...
const screens = {
  intro: $("#intro"), letter: $("#letter"), wish: $("#wish"), gift: $("#gift"),
...
function initScratchCard() {
  ...
  function paint(w, h) {
    ...
    ctx.textAlign = "center";
    ctx.fillText("✦ gores di sini ✦", w / 2, h / 2 + 4);
  }
  size();
  ...
  // canvas glued to the card's real size at all times, not just once on load.
  if ("ResizeObserver" in window) {
    const ro = new ResizeObserver(() => { if (!revealed) size(); });
    ro.observe(card);
  }
  window.addEventListener("resize", () => { if (!revealed) size(); });
  if (document.fonts && document.fonts.ready) {
    document.fonts.ready.then(() => { if (!revealed) size(); });
  }
  ...
  function onDown(e) { if (revealed) return; drawing = true; const p = pos(e); scratchAt(p.x, p.y); }
  function onMove(e) {
...
$("#flameTarget").addEventListener("click", () => { playSfx("whoosh"); finishCelebration(); });
$("#flameTarget").addEventListener("keydown", (e) => {
  if (e.key === "Enter" || e.key === " ") { e.preventDefault(); playSfx("whoosh"); finishCelebration(); }
});
