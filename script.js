...
function showPinMusicFallback() {
  if (pinMusicFallback && currentScreen === "pin") pinMusicFallback.hidden = false;
}
function hidePinMusicFallback() {
  if (pinMusicFallback) pinMusicFallback.hidden = true;
}
function startPinMusic() {
  ...
  pinMusic.play().then(hidePinMusicFallback).catch(() => {
    ...
    // pemutaran otomatis walau sudah ada gesture — tampilkan tombol manual.
