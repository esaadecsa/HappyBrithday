...
const SCREEN_ORDER = ["intro", "playlist", "letter", "wish", "mood", "spirit", "memories", "polaroid", "minigame", "gift", "candle", "flower", "final"];
...
function selectSong(i) {
  ...
  document.querySelectorAll(".playlist-item").forEach((el, idx) => {
    ...
    if (playMark) playMark.textContent = idx === i ? "❚❚" : "▶";
  });
...
function goPlaylist() {
  showScreen("playlist");
  if (selectedSongIndex === null && CONFIG.playlist.length) selectSong(0);
}
...
function primaryAction() {
  ...
  if (currentScreen === "intro") return goPlaylist();
  if (currentScreen === "playlist") return selectedSongIndex !== null && openLetter();
...
document.addEventListener("keydown", (e) => {
  if (["ArrowRight", "Enter", " "].includes(e.key)) {
    if (["intro", "playlist", "letter", "wish", "memories", "polaroid", "flower"].includes(currentScreen)) { e.preventDefault(); primaryAction(); }
  }
...
document.addEventListener("touchend", (e) => {
  ...
  if (Math.abs(dx) > 60 && Math.abs(dx) > Math.abs(dy) * 1.5 && dx < 0) {
    if (["intro", "playlist", "letter", "wish", "memories", "polaroid", "flower"].includes(currentScreen)) primaryAction();
  }
}, { passive: true });
...
$("#sealBtn").onclick = () => {
  ...
  playSfx("pop");
  setTimeout(goPlaylist, 380);
};
$("#openBtn").onclick = () => { playSfx("pop"); goPlaylist(); };
$("#playlistNextBtn").onclick = () => { if (selectedSongIndex === null) return; playSfx("tap"); openLetter(); };
