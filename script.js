...
const CONFIG = {
  name: "kamu",
  // Ini tanggal utama yang dipakai halaman PIN+countdown buat "menunggu".
  // Ganti sesuai tanggal aslinya kalau perlu.
...
/* ---------- pin + countdown, satu halaman ----------
   Selama belum waktunya, #pin nampilin hitung mundur (.pre-unlock).
   PIN di layar ini FUNGSINYA CUMA buat buka paksa lebih awal — begitu
   tanggalnya beneran sampai, kelas "time-unlocked" nyalain .post-unlock
   (tombol "Lanjutkan" polos, nggak perlu PIN lagi di titik ini). */
let capsuleTimer = null;
function initTimeCapsule() {
  const card = $(".pin-inner");
  if (!card) return;
  if (isTimeCapsuleUnlocked()) { card.classList.add("time-unlocked"); return; }
  const label = $("#capsuleCountdown");
  const tick = () => {
    if (isTimeCapsuleUnlocked()) {
      card.classList.add("time-unlocked");
...
function showScreen(name) {
  ...
  /* ---------- arah "pindah tempat" (push navigation) ----------
     Lanjut ke bagian berikutnya di cerita = target masuk dari kanan
     sambil section sebelumnya didorong keluar ke kiri; mundur/ulang
     = kebalikannya. Dihitung dari posisi di SCREEN_ORDER, jadi nggak
     perlu ditandai manual per section — otomatis konsisten di semua
     alur navigasi (tombol Lanjut, panah, slider, dot, keyboard, dst). */
  const orderIndex = (key) => SCREEN_ORDER.indexOf(key === "nope" ? "gift" : key);
  const prevKey = currentScreen;
  const prevEl = screens[prevKey] || document.querySelector(".screen.active");
  const prevIdx = orderIndex(prevKey);
  const targetIdx = orderIndex(name);
  const goingBack = prevIdx >= 0 && targetIdx >= 0 && targetIdx < prevIdx;
  target.style.setProperty("--enter-offset", goingBack ? "-100%" : "100%");
  if (prevEl && prevEl !== target) prevEl.style.setProperty("--enter-offset", goingBack ? "100%" : "-100%");
...
/* ---------- once the date arrives naturally, no PIN needed anymore —
   just continue straight into the playlist screen. ---------- */
$("#pinContinueBtn")?.addEventListener("click", () => {
  playSfx("chime");
  vibrate(16);
  showScreen("playlist");
});
