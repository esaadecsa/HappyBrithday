...
function updateProgress(name) {
  ...
  slider.dataset.maxReach = String(hasBlownCandle ? SCREEN_ORDER.length - 1 : idx);
...
function showScreen(name) {
  ...
  currentScreen = name;
  document.body.classList.remove(...SCREEN_ORDER.map((s) => `screen-${s}`));
...
// Ambient parallax: very subtle on touch/desktop so the background feels alive
// without stealing attention from the story.
(function initAmbientMotion() {
  const aurora = $("#ambientAurora");
  if (!aurora) return;
  let raf = 0, px = 0, py = 0;
  function paint() {
    raf = 0;
    aurora.style.setProperty("--mx", `${px * 18}px`);
    aurora.style.setProperty("--my", `${py * 14}px`);
  }
  function move(x, y) {
    px = (x / Math.max(1, innerWidth) - .5);
    py = (y / Math.max(1, innerHeight) - .5);
    if (!raf) raf = requestAnimationFrame(paint);
  }
  window.addEventListener("pointermove", e => move(e.clientX, e.clientY), { passive:true });
  window.addEventListener("deviceorientation", e => {
    if (e.gamma == null || e.beta == null) return;
    move(innerWidth * (.5 + Math.max(-.35, Math.min(.35, e.gamma / 90))),
         innerHeight * (.5 + Math.max(-.25, Math.min(.25, (e.beta - 35) / 90))));
  }, { passive:true });
})();
...
function confetti() {
  ...
  const colors = ["#ffb8d1", "#ffd9e6", "#ef8fae", "#faeef2", "#e3a6bd", "#ffb15a"];
  for (let i = 0; i < 150; i++) {
...
function burstMiniConfetti(anchor, colors) {
  ...
  const layer = $("#confetti");
  const palette = colors || ["#ffb8d1", "#ffd9e6", "#ef8fae", "#faeef2", "#e3a6bd", "#ffb15a"];
  for (let i = 0; i < 26; i++) {
    const e = document.createElement("i"); e.className = "confetti";
...
function nextPinRequestDate() {
  ...
  let target = new Date(`${year}-09-01T00:00:00+07:00`);
  if (target <= now) target = new Date(`${year + 1}-09-01T00:00:00+07:00`);
  return target;
}
...
function finishCelebration() {
  ...
  hasBlownCandle = true;
  $("#progress")?.classList.add("nav-unlocked");
...
function navigateToIndex(idx, fromSlider = false) {
  ...
  const maxReach = hasBlownCandle ? SCREEN_ORDER.length - 1 : Math.max(0, currentIdx);
  if (idx > maxReach) {
    const slider = $("#progressSlider");
...
if (progressSlider) {
  progressSlider.addEventListener("input", (e) => {
    ...
    const maxReach = hasBlownCandle ? SCREEN_ORDER.length - 1 : Math.max(0, currentIdx);
    if (idx > maxReach) {
      e.target.value = String(maxReach);
      updateProgress(currentScreen);
      return;
    }
    const pct = (idx / (SCREEN_ORDER.length - 1)) * 100;
    $("#sliderFill")?.style.setProperty("width", pct + "%");
    $("#sliderHeart")?.style.setProperty("--slider-progress", pct + "%");
    const label = $("#progressLabel");
    const counter = $("#progressCounter");
    const pretty = SCREEN_ORDER[idx].replace("minigame", "MINI GAME").replace("polaroid", "MEMORIES").replace("farewell", "LAST LETTER").replace("flower", "FLOWERS").replace("reaction", "REACTION").replace("reply", "YOUR REPLY").toUpperCase();
    if (label) label.textContent = pretty;
    if (counter) counter.textContent = `${String(idx + 1).padStart(2, "0")} / ${String(SCREEN_ORDER.length).padStart(2, "0")}`;
