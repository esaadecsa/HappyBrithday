...
async function startMic(){
  ...
  // mobile browsers (iOS Safari especially) the "user gesture" flag is lost
  // once an await happens, so a context created after getUserMedia can stay
  // suspended forever and the mic never registers anything. Doing it first
  // fixes that.
  try{
    if(!audioCtx) audioCtx=new (window.AudioContext||window.webkitAudioContext)();
    if(audioCtx.state==="suspended") await audioCtx.resume();
  }catch(e){/* we retry resume again below once the stream is ready */}
  try{
    ...
    blowLoop=setInterval(()=>{
      ...
      micLastRms=rms;
      // Live feedback so the person can actually see the mic is listening,
      // instead of staring at a static "sudah siap" message.
      if(performance.now()-startedAt>350 && !celebrationDone){
        const level=Math.max(1,Math.min(5,Math.round(rms*45)));
        status.textContent="mendengarkan "+"●".repeat(level)+"○".repeat(5-level)+" · tiup lebih kuat ke mic 💨";
      }
      // Blow = sustained air noise, not just one tap/click. Thresholds
      // loosened slightly so quieter phone mics still register a real blow.
      if(rms>.055 && (peak>.22 || spike>.018)) strong++;
      else strong=Math.max(0,strong-1);
      if(performance.now()-startedAt>300 && strong>=3){
        playSfx("whoosh");
        finishCelebration();
      }
    },45);
