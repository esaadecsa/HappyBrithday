...
// Panah kiri selalu boleh mundur satu langkah (kalau ada langkah sebelumnya).
// Panah kanan cuma boleh maju kalau sudah bebas navigasi (lilin sudah
// ditiup) — sebelum itu, disable (mentok, sama kayak dot).
function updateProgressArrows(idx) {
  const prevBtn = $("#progressPrevBtn");
  const nextBtn = $("#progressNextBtn");
  if (!prevBtn || !nextBtn) return;
  prevBtn.disabled = idx <= 0;
  nextBtn.disabled = idx < 0 || idx >= SCREEN_ORDER.length - 1 || !hasBlownCandle;
}
...
document.querySelectorAll("#progress i").forEach((dot) => {
  ...
  const jump = () => {
    // Sebelum lilin ditiup: cuma bagian yang sudah dilewati (atau saat
    // ini) yang bisa diklik — nggak bisa loncat ke depan (mentok).
    // Setelah lilin ditiup sekali: bebas loncat ke halaman mana pun.
    if (!hasBlownCandle && !dot.classList.contains("done") && !dot.classList.contains("on")) return;
...
/* ---------- tombol panah kiri/kanan (prev/next) di samping dot ---------- */
$("#progressPrevBtn")?.addEventListener("click", () => {
  const idx = SCREEN_ORDER.indexOf(currentScreen);
  if (idx <= 0) return;
  const step = SCREEN_ORDER[idx - 1];
  const fn = STEP_GOTO[step];
  playSfx("tap");
  if (fn) fn(); else showScreen(step);
});
$("#progressNextBtn")?.addEventListener("click", () => {
  // Cuma jalan kalau navigasi udah bebas (lilin sudah ditiup) — sebelum
  // itu tombol ini otomatis disabled lewat updateProgressArrows().
  if (!hasBlownCandle) return;
  const idx = SCREEN_ORDER.indexOf(currentScreen);
  if (idx < 0 || idx >= SCREEN_ORDER.length - 1) return;
  const step = SCREEN_ORDER[idx + 1];
  const fn = STEP_GOTO[step];
  playSfx("tap");
  if (fn) fn(); else showScreen(step);
