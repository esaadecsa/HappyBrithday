...
const screens = {
  ...
  nope: $("#nope"), candle: $("#candle"), flower: $("#flower-screen"), farewell: $("#farewell"), final: $("#final")
};
const SCREEN_ORDER = ["playlist", "minigame", "intro", "letter", "wish", "mood", "spirit", "memories", "polaroid", "gift", "candle", "flower", "farewell", "final"];
...
function selectSong(i) {
  ...
  if (music.getAttribute("src") !== song.src) {
    music.src = song.src;
  }
  startMusic();
...
document.addEventListener("keydown", (e) => {
  if (["ArrowRight", "Enter", " "].includes(e.key)) {
    if (["intro", "playlist", "letter", "wish", "memories", "polaroid", "flower", "farewell"].includes(currentScreen)) { e.preventDefault(); primaryAction(); }
  }
...
document.addEventListener("touchend", (e) => {
  ...
  if (Math.abs(dx) > 60 && Math.abs(dx) > Math.abs(dy) * 1.5 && dx < 0) {
    if (["intro", "playlist", "letter", "wish", "memories", "polaroid", "flower", "farewell"].includes(currentScreen)) primaryAction();
  }
}, { passive: true });
...
let spiritIdx = -1;
let arisanState = "idle"; // idle -> popped -> opened
function drawSpirit() {
  let i;
...
$("#arisanJar").onclick = () => {
  if (arisanState !== "idle") return;
  const jar = $("#arisanJar");
  playSfx("tap");
  jar.classList.add("shaking");
  $("#spiritHint").textContent = "lagi dikocok...";
  setTimeout(() => {
    jar.classList.remove("shaking");
    drawSpirit();
    $("#arisanStage").classList.add("popped");
    arisanState = "popped";
    playSfx("chime");
    $("#spiritHint").textContent = "tarik & buka kertasnya ✦";
  }, 620);
};
$("#arisanPaper").onclick = () => {
  if (arisanState !== "popped") return;
  $("#arisanStage").classList.add("opened");
  arisanState = "opened";
  playSfx("chime");
...
$("#spiritAgainBtn").onclick = () => {
  playSfx("tap");
  $("#arisanStage").classList.remove("opened", "popped");
  arisanState = "idle";
  $("#spiritHint").textContent = "kocok kalengnya, terus tarik kertasnya ✦";
};
$("#spiritNextBtn").onclick = () => { playSfx("tap"); goMemories(); };
function resetSpirit() {
  $("#arisanStage").classList.remove("opened", "popped");
  $("#arisanJar").classList.remove("shaking");
  arisanState = "idle";
