...
function initBackground() {
  ...
  for (let i = 0; i < HEART_COUNT; i++) {
    const mat = new THREE.SpriteMaterial({
      map: heartTex,
      color: Math.random() > 0.5 ? 0xff8fb3 : 0xd9b8ff,
      transparent: true,
...
function initGift() {
  ...
  const key = new THREE.PointLight(0xffb8d1, 1.1, 20);
  key.position.set(2, 3, 4);
  scene.add(key);
  const rim = new THREE.PointLight(0x8b6bff, 0.85, 20);
  rim.position.set(-3, -1, -3);
  scene.add(rim);

  const boxMat = new THREE.MeshStandardMaterial({ color: 0xd9498f, roughness: 0.35, metalness: 0.15 });
  const lidMat = new THREE.MeshStandardMaterial({ color: 0x8b6bff, roughness: 0.3, metalness: 0.2 });
...
function initPolaroid() {
  ...
  const cards = items.map((item) => {
    ...
    const frame = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({ color: 0xf4f1ea, roughness: 0.85 })
    );
    cardGroup.add(frame);
